...
export function AkiraShell({
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#01040e] text-white">
      <AnimatedBackground />
...
function TopBar({
  ...
  return (
    <header className="flex items-center justify-between border-b border-white/5 bg-black/30 px-4 py-4 backdrop-blur-xl lg:px-10">
      ...
      {cards
        ...
        .map((card) => (
          <button
            ...
            className={cn(
              "group h-full rounded-[28px] border border-white/10 bg-gradient-to-br p-[1px] text-left",
              card.glow
            )}
          >
            <div
              className={cn(
                "h-full rounded-[26px] bg-black/50 p-5 transition-transform duration-300 group-hover:-translate-y-1 group-hover:bg-black/40",
            ...
            >
              ...
              <div className="mt-6 flex flex-wrap gap-4">
                {card.metrics.map((metric) => (
                  <div
                    key={metric.label}
                    className="rounded-2xl border border-white/20 bg-black/20 px-4 py-2 text-sm text-white/80"
                  >
...
function SlidingPanel({
  ...
  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-y-0 right-0 flex w-full justify-end transition duration-500 ease-out",
        open ? "translate-x-0" : "translate-x-full"
      )}
    >
      <div className="pointer-events-auto flex h-full w-full flex-col border-l border-white/10 bg-[#01040e]/95 p-6 shadow-[0_0_60px_rgba(0,0,0,0.6)] backdrop-blur-2xl lg:w-[55%]">
